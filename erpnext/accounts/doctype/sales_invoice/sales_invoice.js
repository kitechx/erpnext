...
erpnext.accounts.SalesInvoiceController = erpnext.selling.SellingController.extend({
	...
	onload: function() {
		//frappe.show_alert({message: __("onload sales_contoller" ), indicator: 'green'});
		var me = this;
		this._super();
		...
		if(this.frm.doc.__islocal && this.frm.doc.amended_from) {
			//mohamed clears ktx_key_posting_time
			me.frm.set_value("ktx_key_posting_time", "");
			me.frm.refresh_fields();
		}
	...
	refresh: function(doc, dt, dn) {
		...
		// Mohamed Abdelaleem
		//old 
		cur_frm.add_custom_button(__('Send to Kitchen'), () => {
			me.set_restaurant_table_status(me.frm, "Kitchen");
			frappe.run_serially([
					//() => frappe.click_button('Save'),	
					//() => cur_frm.refresh(),
					() => 	{
							me.frm.doc.kitchen_print_flag	= 0;
							me.frm.doc.beverage_print_flag	= 0;
							},
					() => 	me.check_sent_to_printer(me.frm),
					() => 	{if (me.frm.doc.kitchen_print_flag == 1 ) me.print_forms(me.frm,'Kitchen', 'Sales Invoice')},
					() => 	{if (me.frm.doc.beverage_print_flag  == 1) me.print_forms(me.frm,'Beverage', 'Sales Invoice')},
					//() =>	{if (me.frm.doc.beverage_print_flag  == 1) me.set_sent_to_printer (me.frm,'Beverage')},
					() => 	{if (me.frm.doc.kitchen_print_flag == 1) me.set_sent_to_printer (me.frm,'')} 
			]);
		});
		

		cur_frm.add_custom_button(__('Reciept'), () => {
			me.set_restaurant_table_status(me.frm, "Printed");
			me.print_forms(cur_frm,'POS', 'Sales Invoice');
		});
		// Mohamed Abdelaleem
...
//Mohamed Abdelaleem
    ...
    check_sent_to_printer: function(frm) {
        ...
        (frm.doc.items || []).forEach((d) => {
			if ( d.sent_to_printer == 0)
			if ( d.item_group_type == "Kitchen" ) {
				frm.doc.kitchen_print_flag = 1;
			}
			if ( d.sent_to_printer == 0) if ( d.item_group_type == "Beverage" ) {
   ...
   set_restaurant_table_status: function(frm, table_status) {
		//frappe.show_alert({ message: __("set_restaurant_table_status  "+frm.doc.restaurant_table),indicator: 'green'}); 
    ...
    print_forms: function(frm, action_type, doctype) {
		/*frappe.show_alert({
                                message: __('print_forms...'+action_type),
                                indicator: 'green'
                            });*/
                    frappe.call({
                        ...
                        args: {
                            ...
                            doctype: doctype,
                            docname: frm.doc.name/*r.message.name*/,
                            action_type: action_type
                       },
                        callback: function(r) {
                            frappe.show_alert({
                                message: __('Printed...'+action_type),
                                indicator: 'green'
                            });
                        }
                    });
    ...
    set_sent_to_printer: function(frm, set_action_type) {
        // Mohamed abdelaleem
//        frappe.show_alert({ message: __('Inside ....set_sent_to_printer'),indicator: 'red'});
        (frm.doc.items || []).forEach((d) => {
			//if (d.item_group_type == set_action_type ) {
			if (d.sent_to_printer == 0 ) {
				d.sent_to_printer = 1;
			}
//			frappe.show_alert({ message: __("set_sent_to_printer : "+d.item_group_type +"--"+ set_action_type ),indicator: 'red'});
		});
        //Mohamed Abdelaleem
    },
...
frappe.ui.form.on('Sales Invoice', {
	...
	onload_post_render: function(frm) {
		...
		//cur_frm.set_df_property(“btn”, “hidden”, true);
		...
		$input_s.on('keyup', function(e) {
			...
			if (e.which === 13) {
				...
				frappe.call({
					method: 'erpnext.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.get_item_group_type',
					args: {
						item: item
					},
					callback: (r) => {
						if(!r.message) {
							...
							(frm.doc.items || []).forEach((d) => {
								...
								/*if (!d.item_code) {
									frappe.show_alert({message: __("Item code empty " +d.item_code), indicator: 'red'});
									d.item_code =  item,
									d.qty = 1;
									added = true;
									frm.get_field("items").refresh()
									frappe.show_alert({message: __("Item code filled " +d.item_code), indicator: 'red'});
								}*/
								...
								() => {
										if (!added) {
											/*return*/ 
												...
												frappe.model.set_value(si_item.doctype, si_item.name, 'item_code', item);
	...
	onload: function(frm) {
		//frappe.show_alert({message: __("onload 2 sales_invoice " ), indicator: 'green'});
	...
	refresh: function(frm) {
		...
		/*cur_frm.add_custom_button(__('Reciept'), () => {
               return frappe.run_serially([
					...
					() => frm.refresh_fields(),//frm.refresh()	,
...
//new place for functions	mohamed 	
   /*set_restaurant_table_status: function(frm, table_status) {
		if (!frm.doc.restaurant_table) { return};
		frappe.call({
			method: "frappe.client.set_value",
	   ...
	   print_forms: function(frm, action_type, doctype) {
			frappe.show_alert({ message: __("1-print_forms....."+frm.doc.__unsaved +" + "+frm.doc.name),indicator: 'green'})
                    frappe.call({
                        method: 'erpnext.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.ktx_printt_via_printnode',
                        args: {
                            user: frappe.session.user,
                        ...
                        callback: function(r) {
                            frappe.show_alert({message: __('Printed...'+action_type+frm.doc.__unsaved +" + "+frm.doc.name),indicator: 'green'});
                        }
                    });
    }*/
