...
def get_invoices(filters):
	return frappe.db.sql("""select a.name, a.base_grand_total as grand_total,
		...
		where a.docstatus = 1 and concat(a.posting_date, ' ', a.posting_time,':00') >= %(from_date)s
		and concat(a.posting_date, ' ', a.posting_time,':00') <= %(to_date)s and a.company=%(company)s
